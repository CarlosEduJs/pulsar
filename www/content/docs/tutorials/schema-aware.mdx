...
const byName = await db.select({ id: users.id, name: users.name })
  .from(users)
  .where(eq(users.name, "Alice"))    // `name` has no index
  .limit(1);

// 2. Reference a column that does not exist in the schema
const byTypo = await db.select({ contnt: posts.contnt })   // `contnt` != `content`
  .from(posts)
  .limit(1);
...
```bash
pulsar check src/ --format json | jq 'group_by(.rule_id) | map({rule: .[0].rule_id, count: length})'
```
</Step>
</Steps>
...
<Cards>
  <Card title="Next: CI/CD Integration" href="/docs/tutorials/ci-cd" />
</Cards>
