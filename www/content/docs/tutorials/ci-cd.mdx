...
<Callout type="info">
  Pulsar already exits with code 1 when errors or parse failures are found, so the extra jq step is
  only needed if you want custom filtering (e.g., only fail on specific rules).
</Callout>
